...
public class ExternalImageEventListener {
    ...
    public void handleImagePresignedUrlCreated(ImagePresignedUrlCreatedEvent event) {
        log.info("Received ImagePresignedUrlCreatedEvent via RabbitMQ in mopl-dm. key: {}", event.key());
        try {
            ...
            log.info("Successfully updated ExternalImageView for key: {} in mopl-dm", event.key());
        } catch (Exception e) {
            log.error("Failed to handle ImagePresignedUrlCreatedEvent for key: {} in mopl-dm", event.key(), e);
            throw new RuntimeException("Failed to update ExternalImageView in mopl-dm", e);
        }
    }
}
